                        ...
                        <thead class="bg-gray-50 dark:bg-gray-800 text-xs font-semibold uppercase text-gray-700 dark:text-gray-300 border-b border-gray-200 dark:border-gray-700">
                            <tr>
                                ...
                                @if (bouncer()->hasPermission('dropshipping.procurement_v2.cost_view'))
                                    ...
                                    <th class="p-4 text-right">{{ trans('procurement::app.datagrid.total-cost') }}</th>
                                @endif
                            </tr>
                        </thead>
                        <tbody class="divide-y divide-gray-200 dark:divide-gray-800">
                            @forelse ($openDemands as $demand)
                                ...
                                <tr class="hover:bg-gray-50 dark:hover:bg-gray-800/50 transition">
                                    <td class="p-4">
                                        <input type="checkbox" name="demand_ids[]" value="{{ $demand->id }}" class="demand-checkbox rounded border-gray-300 text-blue-600 focus:ring-blue-500" onchange="updateSelectionSummary()">
                                    </td>
                                    ...
                                    @if (bouncer()->hasPermission('dropshipping.procurement_v2.cost_view'))
                                        <td class="p-4 text-right">${{ number_format($unitCost, 2) }}</td>
                                        <td class="p-4 text-right font-semibold text-gray-900 dark:text-white">${{ number_format($lineCost, 2) }}</td>
                                    @endif
                                </tr>
                            @empty
                                <tr>
