<x-admin::layouts>
    ...
    @php
        $initialItems = $deal->products->map(function ($item) {
            return [
                'product_id'     => $item->product_id,
                'flash_price'    => (float) $item->flash_price,
                'allocation_qty' => (int) $item->allocation_qty,
                'sold_qty'       => (int) $item->sold_qty,
            ];
        })->toArray();

        if (empty($initialItems)) {
            $initialItems = [['product_id' => '', 'flash_price' => '', 'allocation_qty' => 50, 'sold_qty' => 0]];
        }
    @endphp
        ...
        <div class="flex items-center justify-between gap-4 border-b dark:border-gray-800 pb-4 mb-6">
            <div>
                <h1 class="text-2xl font-black text-gray-900 dark:text-white flex items-center gap-2">
                    ...
                    <span>تعديل العرض السريع #{{ $deal->id }} - {{ $deal->title }}</span>
                </h1>
                <p class="text-xs text-gray-500 dark:text-gray-400 mt-1">
                    تعديل توقيت العرض، أسعار التخفيض، والكميات المخصصة للمنتجات
                </p>
            </div>
                ...
                <div class="p-6 bg-white dark:bg-gray-900 rounded-2xl shadow-sm border border-gray-200 dark:border-gray-800">
                    ...
                    <x-admin::form.control-group class="mb-4">
                        ...
                        <x-admin::form.control-group.control
                            ...
                            :label="'الحالة'"
                        >
                            <option value="1" {{ $deal->status ? 'selected' : '' }}>مفعّل (Active)</option>
                            <option value="0" {{ !$deal->status ? 'selected' : '' }}>غير مفعّل (Inactive)</option>
                        </x-admin::form.control-group.control>
