        ...
        <div class="flex items-center justify-between flex-wrap gap-4">
            <div class="flex flex-col">
                ...
                <h1 class="text-2xl font-bold text-gray-800 dark:text-white mt-1 flex items-center gap-3">
                    ...
                    @php
                        $statusVal = $manifest->status instanceof \BackedEnum ? $manifest->status->value : $manifest->status;
                        $statusClass = match ($statusVal) {
                            ...
                            'partially_received' => 'bg-cyan-100 text-cyan-800 dark:bg-cyan-950 dark:text-cyan-200',
                            'cancelled' => 'bg-rose-100 text-rose-800 dark:bg-rose-950 dark:text-rose-200',
                            default => 'bg-gray-100 text-gray-800 dark:bg-gray-800 dark:text-gray-200',
                        };
                    @endphp
                    <span class="text-xs px-2.5 py-0.5 rounded font-semibold {{ $statusClass }}">
                        {{ trans("inventory::app.admin.transfer_statuses.{$statusVal}") ?: $statusVal }}
                    </span>
                </h1>
            </div>
            ...
            <div class="flex items-center gap-3">
                ...
                @if(($manifest->status->value ?? $manifest->status) === 'draft')
                    <form method="POST" action="{{ route('admin.inventory.transfers.cancel', $manifest->id) }}">
                        @csrf
                        <button type="submit" class="secondary-button text-rose-600 hover:text-rose-700 hover:bg-rose-50 border-rose-300 dark:border-rose-800 dark:hover:bg-rose-950/30" onclick="return confirm('{{ trans('inventory::app.admin.transfers.cancel-confirm') }}')">
                            ✕ {{ trans('inventory::app.admin.transfers.cancel-action') }}
                        </button>
                    </form>
            ...
            <div class="p-5 bg-white dark:bg-gray-900 rounded-lg border border-gray-200 dark:border-gray-800 shadow-sm flex flex-col gap-3">
                ...
                <div class="flex flex-col gap-2 text-xs">
                    <div class="flex justify-between">
                        ...
                        <span class="font-bold text-gray-900 dark:text-white">{{ $manifest->sourceInventorySource?->name ?: ($manifest->source?->name ?: '-') }} ({{ $manifest->sourceInventorySource?->country ?: ($manifest->source?->country ?: '-') }})</span>
                    </div>
                    <div class="flex justify-between">
                        <span class="text-gray-500">الوجهة:</span>
                        <span class="font-bold text-gray-900 dark:text-white">{{ $manifest->destinationInventorySource?->name ?: ($manifest->destination?->name ?: '-') }} ({{ $manifest->destinationInventorySource?->country ?: ($manifest->destination?->country ?: '-') }})</span>
                    </div>
                    <div class="flex justify-between">
                    ...
                    @if($manifest->notes)
                        <div class="flex flex-col gap-1 border-t pt-2 mt-1">
                            <span class="text-gray-500 font-medium">الملاحظات:</span>
                            <span class="text-gray-700 dark:text-gray-300 whitespace-pre-line">{{ $manifest->notes }}</span>
                        </div>
                    @endif
                </div>
            </div>
                    ...
                    <table class="w-full text-xs text-right">
                        ...
                        <tbody class="divide-y divide-gray-100 dark:divide-gray-800">
                            @foreach($manifest->items as $item)
                                @php
                                    $itemCond = $item->item_condition instanceof \BackedEnum ? $item->item_condition->value : $item->item_condition;
                                @endphp
                                <tr>
