...
<div class="mt-6 pt-6 border-t border-gray-200 dark:border-gray-800" id="wallet-withdrawal-methods-container">
    ...
    <div class="border border-gray-200 dark:border-gray-800 rounded-md overflow-hidden bg-white dark:bg-gray-900">
        ...
        <div id="wm-list" class="divide-y divide-gray-100 dark:divide-gray-800">
            @forelse ($initialMethods as $m)
                <div class="p-3 flex items-center justify-between hover:bg-gray-50 dark:hover:bg-gray-800/50 transition-colors" id="wm-row-{{ $m->id }}">
                    ...
                    <div class="flex items-center gap-1" id="wm-actions-{{ $m->id }}">
                        <button type="button" onclick="startEditWm({{ $m->id }}, '{{ addslashes($m->name) }}')" class="px-2.5 py-1 text-xs font-bold text-blue-600 hover:bg-blue-50 dark:text-blue-400 rounded">
                            تعديل
                        </button>
                        <button type="button" onclick="toggleWm({{ $m->id }})" class="px-2.5 py-1 text-xs font-bold {{ $m->status ? 'text-amber-600 hover:bg-amber-50 dark:text-amber-400' : 'text-emerald-600 hover:bg-emerald-50 dark:text-emerald-400' }} rounded">
                            {{ $m->status ? 'إيقاف' : 'تفعيل' }}
                        </button>
                        <button type="button" onclick="openWmDeleteModal({{ $m->id }}, '{{ addslashes($m->name) }}')" class="px-2.5 py-1 text-xs font-bold text-rose-600 hover:bg-rose-50 dark:text-rose-400 rounded">
                            حذف
                        </button>
                    </div>
                </div>
...
<script>
    ...
    function addWithdrawalMethod() {
        ...
        fetch("{{ route('admin.wallet.withdrawal_methods.store') }}", {
            method: 'POST',
            headers: {
    ...
    function saveEditWm(id) {
        ...
        fetch(`{{ route('admin.wallet.withdrawal_methods.index') }}/${id}`, {
            method: 'PUT',
            headers: {
                'Content-Type': 'application/json',
                'X-CSRF-TOKEN': '{{ csrf_token() }}',
    ...
    function toggleWm(id) {
        fetch(`{{ route('admin.wallet.withdrawal_methods.index') }}/${id}/toggle`, {
            method: 'POST',
            headers: {
                'Content-Type': 'application/json',
                'X-CSRF-TOKEN': '{{ csrf_token() }}',
    ...
    function confirmDeleteWm() {
        ...
        fetch(`{{ route('admin.wallet.withdrawal_methods.index') }}/${deleteId}`, {
            method: 'DELETE',
            headers: {
                'Content-Type': 'application/json',
                'X-CSRF-TOKEN': '{{ csrf_token() }}',
