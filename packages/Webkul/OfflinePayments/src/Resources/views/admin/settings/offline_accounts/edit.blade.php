<x-admin::layouts>
    <x-slot:title>
        @lang('offline_payments::app.admin.form.edit-title')
    </x-slot>

    <x-admin::form
        method="PUT"
        :action="route('admin.settings.offline_accounts.update', $account->id)"
            ...
            <div class="flex flex-1 flex-col gap-4 max-xl:flex-auto">
                <div class="box-shadow rounded bg-white p-4 dark:bg-gray-900">
                    ...
                    <x-admin::form.control-group>
                        ...
                        <x-admin::form.control-group.control
                            ...
                            name="recipient_name"
                            rules="required"
                            value="{{ old('recipient_name') ?? $account->recipient_name }}"
                ...
                <div class="box-shadow rounded bg-white p-4 dark:bg-gray-900">
                    ...
                    <v-offline-destinations-repeater
                        ...
                        :initial-destinations="{{ json_encode(old('destinations') ?? $account->destinations) }}"
                    ></v-offline-destinations-repeater>
                </div>
            </div>
                        ...
                        <x-admin::form.control-group>
                            ...
                            >
                                @foreach ($channels as $channel)
                                    <option value="{{ $channel->id }}" {{ in_array($channel->id, old('channel_ids') ?? $account->channel_ids ?? []) ? 'selected' : '' }}>
                                        {{ $channel->name }}
                                    </option>
                                @endforeach
                            </x-admin::form.control-group.control>
                        ...
                        <x-admin::form.control-group>
                            ...
                            @if ($account->logo_path)
                                <div class="mb-2 flex items-center gap-2">
                                    <img src="{{ Storage::url($account->logo_path) }}" class="h-12 w-12 rounded object-cover border" alt="Logo">
                                </div>
                            @endif
    ...
    @pushOnce('scripts')
        ...
        >
            <div>
                ...
                >
                    <input type="hidden" :name="'destinations[' + index + '][id]'" :value="dest.id" v-if="dest.id" />
        ...
        <script type="module">
            app.component('v-offline-destinations-repeater', {
                ...
                data() {
                    return {
                        destinations: this.initialDestinations.length > 0
                            ? JSON.parse(JSON.stringify(this.initialDestinations))
                            : [{ currency_id: '', account_identifier: '', swift_code: '', transfer_instructions: '', sort_order: 0 }]
                    };
                },
                methods: {
                    addDestination() {
