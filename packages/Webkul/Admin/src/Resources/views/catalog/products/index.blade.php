        ...
        <template #body="{
            ...
            <template v-else>
                ...
                >
                    <!-- Parent Product Row -->
                    <div
                        class="row px-2 py-2.5 transition-all hover:bg-gray-50 dark:hover:bg-gray-950 sm:px-4 md:grid md:grid-cols-[2fr_1fr_1fr] md:grid-rows-1 md:gap-1.5 cursor-pointer"
                        @click="record.variants && record.variants.length ? record.is_expanded = !record.is_expanded : {}"
                    >
                        <!-- Mobile Layout -->
                            ...
                            <div class="flex items-start justify-between gap-3">
                                <div class="flex items-start gap-2.5">
                                    ...
                                    <div 
                                        ...
                                        class="text-xl text-gray-600 dark:text-gray-300 transition-all duration-200 cursor-pointer self-center"
                                        :class="[record.is_expanded ? 'icon-down-stat' : 'icon-sort-right']"
                                        @click.stop="record.is_expanded = !record.is_expanded"
                                    ></div>
                        ...
                        <div class="hidden md:contents">
                            ...
                            <div class="flex gap-2.5">
                                ...
                                <div 
                                    v-else-if="record.type === 'configurable'"
                                    class="w-[24px]"
                                ></div>
                    ...
                    <div
                        v-if="record.is_expanded && record.variants && record.variants.length"
                        class="bg-gray-50/50 dark:bg-gray-900/50 border-t dark:border-gray-800"
                    >
                        <div
                            v-for="variant in record.variants"
                            class="row border-b border-gray-100 dark:border-gray-800 px-2 py-3 transition-all hover:bg-gray-100/70 dark:hover:bg-gray-950/70 sm:px-4 md:grid md:grid-cols-[2fr_1fr_1fr] md:grid-rows-1 md:gap-1.5 ltr:pl-10 rtl:pr-10"
                        >
                            <!-- Variant Mobile Layout -->
                            <div class="block space-y-2 md:hidden ltr:pl-4 rtl:pr-4">
                                <div class="flex items-start gap-2.5">
                                    <div class="relative flex-shrink-0">
                                        <img
                                            v-if="variant.base_image"
                                            class="h-10 w-10 rounded object-cover"
                                            :src='variant.base_image'
                                        />
                                        <div v-else class="relative h-10 w-10 rounded border border-dashed border-gray-300 dark:border-gray-800 flex items-center justify-center">
                                            <span class="icon-camera text-gray-400"></span>
                                        </div>
                                    </div>
                                    <div class="flex flex-col gap-0.5 flex-1">
                                        ...
                                        <p class="text-[10px] text-gray-500">
                                            SKU: @{{ variant.sku }} | ID: @{{ variant.product_id }}
                                        </p>
                                        <p class="text-xs font-semibold text-gray-700 dark:text-gray-200">
                                            @{{ variant.price_formatted }}
                                        </p>
                                        <p class="text-xs">
                                            <span v-if="variant.quantity > 0" class="text-green-600">
                                                @{{ "@lang('admin::app.catalog.products.index.datagrid.qty-value')".replace(':qty', variant.quantity) }}
                                            </span>
                                            <span v-else class="text-red-600">
                                                @lang('admin::app.catalog.products.index.datagrid.out-of-stock')
                                            </span>
                                        </p>
                                        <p :class="[variant.status ? 'label-active': 'label-info']" class="w-max">
                                            @{{ variant.status ? "@lang('admin::app.catalog.products.index.datagrid.active')" : "@lang('admin::app.catalog.products.index.datagrid.disable')" }}
                                        </p>
                                    </div>
                                </div>
                                <div class="flex items-center gap-1 justify-end">
                                    <span
                                        class="cursor-pointer rounded-md p-1 text-xl transition-all hover:bg-gray-200 dark:hover:bg-gray-800"
                                        :class="action.icon"
                                        v-text="! action.icon ? action.title : ''"
                                        v-for="action in variant.actions"
                                        @click.stop="performAction(action)"
                                    >
                                    </span>
                                </div>
                            ...
                            <div class="hidden md:contents">
                                ...
                                <div class="flex gap-2.5 items-center">
                                    ...
                                    <div class="flex flex-col gap-1">
                                        <p class="break-all text-sm font-semibold text-gray-700 dark:text-gray-200">
                                            @{{ variant.name }}
                                        </p>
                                        <p class="text-xs text-gray-500">
                                            @{{ "@lang('admin::app.catalog.products.index.datagrid.sku-value')".replace(':sku', variant.sku) }}
                                        </p>
                                ...
                                <div class="flex gap-1.5">
                                    <div class="relative">
                                        <img
                                            v-if="variant.base_image"
                                            class="max-h-[50px] min-h-[50px] min-w-[50px] max-w-[50px] rounded object-cover"
                                            :src='variant.base_image'
                                        />
                                        <div v-else class="h-[50px] w-[50px] rounded border border-dashed border-gray-300 dark:border-gray-800 flex items-center justify-center">
                                            <span class="icon-camera text-gray-400"></span>
                                        </div>
                                    </div>
                                    ...
                                    <div class="flex flex-col gap-1">
                                        ...
                                        <p class="text-xs text-gray-400">
                                            @{{ "@lang('admin::app.catalog.products.index.datagrid.id-value')".replace(':id', variant.product_id) }}
                                        </p>
                                    </div>
                                </div>
                                ...
                                <div class="flex items-center justify-between gap-x-4">
                                    <div class="flex flex-col gap-1">
                                        <p :class="[variant.status ? 'label-active': 'label-info']">
                                            @{{ variant.status ? "@lang('admin::app.catalog.products.index.datagrid.active')" : "@lang('admin::app.catalog.products.index.datagrid.disable')" }}
                                        </p>
                                    </div>
