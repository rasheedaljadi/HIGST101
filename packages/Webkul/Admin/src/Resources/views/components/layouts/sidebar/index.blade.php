            ...
            @foreach (menu()->getItems('admin') as $menuItem)
                ...
                >
                    <a
                        ...
                        class="flex gap-2.5 p-1.5 items-center cursor-pointer hover:rounded-lg {{ $menuItem->isActive() == 'active' ? 'hayest-nav-active rounded-lg' : ' hover:bg-gray-100 hover:dark:bg-gray-950' }} peer"
                    >
                        <span class="{{ $menuItem->getIcon() }} text-2xl {{ $menuItem->isActive() ? '!text-gold-500' : ''}}"></span>
