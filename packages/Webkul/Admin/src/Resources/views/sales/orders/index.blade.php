                ...
                <div class="row grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 items-center border-b px-2 sm:px-4 py-2.5 dark:border-gray-800">
                    <div
                        ...
                        v-for="(columnGroup, index) in [
                            { title: '@lang('admin::app.sales.orders.index.datagrid.order-information')', columns: ['increment_id', 'created_at', 'status'] },
                            { title: '@lang('admin::app.sales.orders.index.datagrid.payment-information')', columns: ['base_grand_total', 'method', 'channel_id'] },
                            { title: '@lang('admin::app.sales.orders.index.datagrid.customer-information')', columns: ['full_name', 'customer_email', 'location'] },
                            { title: '@lang('admin::app.sales.orders.index.datagrid.items')', columns: ['items'] }
                        ]"
                    >
                        <p class="text-gray-600 dark:text-gray-300 text-sm sm:text-base">
                            <span
                                :class="{
                                    'font-medium text-gray-800 dark:text-white': columnGroup.columns.includes(applied.sort.column),
                                    'cursor-pointer hover:text-gray-800 dark:hover:text-white': columnGroup.columns.some(column => available.columns.find(columnTemp => columnTemp.index === column)?.sortable),
                                }"
                                @click="
                                    () => {
                                        const sortableColumn = columnGroup.columns
                                            .map(column => available.columns.find(columnTemp => columnTemp.index === column))
                                            .find(columnTemp => columnTemp?.sortable);
                            ...
                            <i
                                ...
                                v-if="columnGroup.columns.includes(applied.sort.column)"
                            >
                            </i>
                        </p>
                    </div>
