...
Route::controller(ConfigurationController::class)->group(function () {
    Route::get('configuration', 'index')->name('admin.configuration.index');

    Route::get('configuration/{slug}/{slug2}/{path}', 'download')->name('admin.configuration.download');

    Route::get('configuration/{slug}/{slug2}', 'index')->name('admin.configuration.index.slug');

    Route::post('configuration', 'store')->name('admin.configuration.store');

    Route::post('configuration/{slug}/{slug2}', 'store')->name('admin.configuration.store.slug');
});
