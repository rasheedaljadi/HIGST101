...
class OrderDataGrid extends DataGrid
{
    ...
    public function prepareQueryBuilder()
    {
        $queryBuilder = DB::table('orders')
            ...
            ->selectRaw('GROUP_CONCAT(order_payment.method SEPARATOR "|") as method')
            ->selectRaw('CONCAT(orders.customer_first_name, " ", orders.customer_last_name) as full_name')
            ->selectRaw('CONCAT(order_address_billing.city, ", ", order_address_billing.state, ", ", order_address_billing.country) as location')
            ->groupBy('orders.id');
    ...
    public function prepareColumns()
    {
        ...
        $this->addColumn([
            ...
            'filterable_options' => [
                ...
                [
                    'label' => trans('admin::app.sales.orders.index.datagrid.accepted'),
                    'value' => Order::STATUS_ACCEPTED,
                ],
                [
            ...
            'closure' => function ($row) {
                switch ($row->status) {
                    ...
                    case Order::STATUS_ACCEPTED:
                        return '<p class="label-active">'.trans('admin::app.sales.orders.view.accepted').'</p>';
