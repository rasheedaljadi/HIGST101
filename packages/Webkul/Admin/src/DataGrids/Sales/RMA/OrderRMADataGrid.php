...
class OrderRMADataGrid extends DataGrid
{
    ...
    public function prepareQueryBuilder(): Builder
    {
        $rmaItemsSubquery = DB::table('rma_items')
            ->selectRaw('order_item_id, SUM(quantity) as total_rma_qty')
        ...
        $queryBuilder = DB::table('orders')
            ...
            ->selectRaw('SUM(order_items.qty_ordered) as total_qty_ordered')
            ->selectRaw('COALESCE(SUM(rma_items_agg.total_rma_qty), 0) as total_rma_qty')
        ...
        $queryBuilder->whereNotNull('order_items.rma_return_period')
            ->whereRaw('DATEDIFF(NOW(), order_items.created_at) <= order_items.rma_return_period');

        $queryBuilder->groupBy('orders.id')
            ->havingRaw('SUM(order_items.qty_ordered) > COALESCE(SUM(rma_items_agg.total_rma_qty), 0)');
