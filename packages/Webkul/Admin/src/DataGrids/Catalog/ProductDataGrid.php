...
class ProductDataGrid extends DataGrid
{
    ...
    protected function processRequest(): void
    {
        ...
        if ($ids) {
            // Use parameter binding for FIELD() to prevent SQL injection
            $placeholders = implode(',', array_fill(0, count($ids), '?'));
            $this->queryBuilder
                ->orderByRaw('FIELD(product_flat.product_id, '.$placeholders.')', $ids);
        }
    ...
    protected function formatRecords($records): mixed
    {
        ...
        foreach ($records as $record) {
            ...
            if ($record->type === 'configurable') {
                ...
                $variants = DB::table('product_flat')
                    ->leftJoin('product_images', 'product_flat.product_id', '=', 'product_images.product_id')
                    ->leftJoin('product_inventories', 'product_flat.product_id', '=', 'product_inventories.product_id')
                    ->select(
                        'product_flat.product_id',
                    ...
                    ->addSelect(DB::raw('SUM(DISTINCT '.$tablePrefix.'product_inventories.qty) as quantity'))
                    ->where('product_flat.parent_id', $record->product_id)
                    ->where('product_flat.locale', app()->getLocale())
                    ->groupBy('product_flat.product_id')
                    ->get();

                foreach ($variants as $variant) {
                    $variant->base_image = $variant->base_image ? Storage::url($variant->base_image) : null;
                    $variant->price_formatted = core()->formatBasePrice($variant->price);
                ...
                $record->variants = $variants->toArray();
            }
        }
