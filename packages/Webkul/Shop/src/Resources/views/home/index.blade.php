    ...
    @foreach ($customizations as $customization)
        ...
        @switch ($customization->type)
            @case ($customization::IMAGE_CAROUSEL)
                ...
                <div class="py-6 md:py-12 bg-[#F6F8FC] dark:bg-gray-950 overflow-hidden">
                    <div class="max-w-[1320px] mx-auto px-3 sm:px-4 md:px-6">
                        <div class="grid grid-cols-1 lg:grid-cols-12 gap-6 md:gap-8 items-stretch">
                            ...
                            <div class="lg:col-span-7 flex flex-col justify-between relative">
                                ...
                                <div class="mb-6 md:mb-8 relative z-10 text-right">
                                    <h2 class="text-2xl md:text-3xl font-extrabold text-[#002060] dark:text-white mb-2">
                                        تسوق حسب الفئة
                                    </h2>
                                ...
                                <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3 sm:gap-4 relative z-10">
                                    @foreach ($customCategories as $cat)
                                        <a href="{{ $cat['link'] }}" class="bg-white dark:bg-gray-900 rounded-2xl shadow-[0_4px_25px_rgba(0,0,0,0.03)] hover:shadow-[0_8px_35px_rgba(0,0,0,0.07)] transition-all duration-300 p-3 sm:p-4 flex items-center justify-between border border-gray-100 dark:border-gray-800 relative group overflow-hidden">
                                            
                                            <!-- Round image cut-out -->
                                            <div class="relative w-16 h-16 sm:w-20 sm:h-20 rounded-full overflow-hidden bg-[#F2F6FC] dark:bg-gray-800 flex items-center justify-center flex-shrink-0">
                                                ...
                                                <div class="absolute -top-0.5 -right-0.5 w-6 h-6 sm:w-7 sm:h-7 rounded-full bg-[#FFC000] flex items-center justify-center shadow-sm">
                                                    {!! $cat['icon_svg'] !!}
                                                </div>
                                            </div>
                                            
                                            <!-- Category Name & Left arrow inside card -->
                                            <div class="flex-1 pr-3 sm:pr-4 flex flex-col items-start justify-center text-right">
                                                <span class="text-sm sm:text-base font-bold text-gray-800 dark:text-gray-100 group-hover:text-[#002060] dark:group-hover:text-[#FFC000] transition-colors duration-300">{{ $cat['name'] }}</span>
                                                <span class="text-[#002060] dark:text-[#FFC000] text-lg sm:text-xl font-bold mt-0.5 sm:mt-1 group-hover:-translate-x-1 transition-transform duration-300">←</span>
                                            </div>
                                ...
                                <!-- View All Button -->
                                <div class="mt-6 md:mt-8 text-center sm:text-right relative z-10">
                                    <a href="{{ route('shop.home.index') }}" class="inline-flex items-center justify-center gap-2.5 bg-[#002060] hover:bg-[#001040] text-white px-6 sm:px-8 py-3 sm:py-3.5 rounded-full hover:shadow-lg transition-all duration-300 font-bold text-sm sm:text-base">
                            ...
                            <div class="lg:col-span-5">
                                <div class="bg-gradient-to-br from-[#0B2562] to-[#123E8E] rounded-3xl md:rounded-[2.5rem] p-4 sm:p-6 md:p-8 text-white flex flex-col justify-between shadow-xl relative overflow-hidden h-full min-h-[420px] sm:min-h-[520px]">
                                    ...
                                    <div class="text-center mb-4 sm:mb-6">
                                        <h3 class="text-2xl sm:text-3xl font-extrabold mb-2 tracking-wide leading-tight">
                                        ...
                                        <a href="{{ route('shop.product_or_category.index', 'apparel-accessories-ar') }}" class="bg-[#FFB900] hover:bg-[#FFC000] text-gray-900 font-bold px-5 sm:px-6 py-1.5 sm:py-2 rounded-full inline-flex items-center justify-center gap-2 mt-3 sm:mt-4 transition-all duration-300 shadow-md text-xs sm:text-sm">
                                            <span>تسوق الآن</span>
                                    ...
                                    <!-- 3 Product Cards -->
                                    <div class="grid grid-cols-1 sm:grid-cols-3 gap-2.5 sm:gap-3 my-3 sm:my-4">
                                        @foreach ($featuredProducts as $index => $prod)
                                            ...
                                            @endphp
                                            <a href="{{ $prodUrl }}" class="bg-white rounded-2xl sm:rounded-3xl p-2.5 sm:p-3 shadow-sm hover:shadow-md transition-all duration-300 flex sm:flex-col justify-between items-center sm:items-stretch relative group text-gray-900">
                                                
                                                <!-- Heart Badge top-left -->
                                                <div class="absolute top-2 left-2 w-6 h-6 sm:w-7 sm:h-7 rounded-full bg-[#FFC000]/10 hover:bg-[#FFC000]/25 text-[#FFC000] flex items-center justify-center cursor-pointer transition-colors duration-200">
                                                ...
                                                <!-- Image -->
                                                <div class="w-16 h-16 sm:w-full sm:h-24 rounded-xl sm:rounded-2xl overflow-hidden bg-gray-50 flex items-center justify-center mb-0 sm:mb-2 shrink-0">
                                                    <img src="{{ $prodImageUrl }}" class="w-full h-full object-contain p-1 group-hover:scale-105 transition-transform duration-300" alt="{{ $prod->name }}">
                                                </div>

                                                <!-- Price and metadata -->
                                                <div class="text-right flex flex-col justify-end flex-1 pr-3 sm:pr-0">
                                                    <span class="text-xs sm:text-sm font-extrabold text-[#002060] block leading-tight">{{ $currentPrice }}</span>
                                                    @if ($hasDiscount)
                                                        <span class="text-[9px] sm:text-[10px] text-gray-400 line-through block leading-none">{{ $originalPrice }}</span>
                                                    @endif
                                                    
                                                    <span class="text-[9px] text-gray-500 block font-semibold mt-0.5 sm:mt-1">{{ $totalSales }} تم البيع</span>
                                                    
                                                    <div class="flex items-center justify-end gap-0.5 text-[9px] sm:text-[10px] mt-0.5">
                                                        <span class="font-bold text-gray-700">{{ $averageRating }}</span>
                                                        <svg class="w-2.5 h-2.5 sm:w-3 sm:h-3 text-[#FFC000] fill-current" viewBox="0 0 24 24">
                                    ...
                                    <div class="grid grid-cols-3 gap-1.5 border-t border-white/20 pt-3 sm:pt-4 mt-2 sm:mt-3">
                                        <div class="flex flex-col items-center text-center">
                                            <svg class="w-4 h-4 sm:w-5 sm:h-5 text-[#FFC000] mb-0.5 sm:mb-1" fill="none" stroke="currentColor" stroke-width="2.5" viewBox="0 0 24 24">
                                                <path stroke-linecap="round" stroke-linejoin="round" d="M18.364 5.636l-3.536 3.536m0 5.656l3.536 3.536M9.172 9.172L5.636 5.636m3.536 9.192l-3.536 3.536M21 12a9 9 0 11-18 0 9 9 0 0118 0zm-5 0a4 4 0 11-8 0 4 4 0 018 0z"/>
                                            </svg>
                                            <span class="text-[10px] sm:text-[11px] font-bold text-white leading-tight">دعم 24/7</span>
                                            <span class="text-[8px] sm:text-[9px] text-white/70">نحن هنا للمساعدة</span>
                                        </div>
                                        <div class="flex flex-col items-center text-center">
                                            <svg class="w-4 h-4 sm:w-5 sm:h-5 text-[#FFC000] mb-0.5 sm:mb-1" fill="none" stroke="currentColor" stroke-width="2.5" viewBox="0 0 24 24">
                                                <path stroke-linecap="round" stroke-linejoin="round" d="M9 12l2 2 4-4m5.618-4.016A11.955 11.955 0 0112 2.944a11.955 11.955 0 01-8.618 3.04A12.02 12.02 0 003 9c0 5.591 3.824 10.29 9 11.622 5.176-1.332 9-6.03 9-11.622 0-1.042-.133-2.052-.382-3.016z"/>
                                            </svg>
                                            <span class="text-[10px] sm:text-[11px] font-bold text-white leading-tight">ضمان الجودة</span>
                                            <span class="text-[8px] sm:text-[9px] text-white/70">منتجات أصلية</span>
                                        </div>
                                        <div class="flex flex-col items-center text-center">
                                            <svg class="w-4 h-4 sm:w-5 sm:h-5 text-[#FFC000] mb-0.5 sm:mb-1" fill="none" stroke="currentColor" stroke-width="2.5" viewBox="0 0 24 24">
                                                <path stroke-linecap="round" stroke-linejoin="round" d="M8 7h12m0 0l-4-4m4 4l-4 4m0 6H4m0 0l4 4m-4-4l4-4"/>
                                            </svg>
                                            <span class="text-[10px] sm:text-[11px] font-bold text-white leading-tight">شحن سريع</span>
                                            <span class="text-[8px] sm:text-[9px] text-white/70">توصيل سريع</span>
                                        </div>
                                    </div>
                                </div>
                            </div>
