...
class ProductRepository extends Repository
{
        ...
        $query = $this->with([
            ...
            $qb = $query->distinct()
                ...
                ->leftJoin('products as variants', function ($join) {
                    $join->on(DB::raw('COALESCE(variants.parent_id, variants.id)'), '=', 'products.id');
                })
    ...
    public function searchFromElastic(array $params = [])
    {
        ...
        $query = $this->with([
            ...
            $placeholders = implode(',', array_fill(0, count($indices['ids']), '?'));
            $qb->orderByRaw('FIELD(products.id, '.$placeholders.')', $indices['ids']);
