# عقد التصميم الفني والمعماري لعروض وحوافز المحفظة الرقمية — الإصدار 1.6
**المشروع:** منصة التجارة والمحفظة الرقمية Bagisto 2.4.x  
**الملف:** `WALLET_PROMOTIONS_DESIGN_CONTRACT_V1.6.md`  
**الحالة:** مسودة عقد تصميم فني وتنفيذي شامل قيد المراجعة والاعتماد (Pre-Implementation Technical Contract - V1.6)  
...
### 1.2. سياسة `unclassified_balance` والمعادلات المحاسبية الموحدة:
- **القرار المالي الصارم:** **عزل وحظر استخدام `unclassified_balance` كلياً عن الشراء والسحب النقدي طالما أن حالة الحساب `backfill_status = 'pending_review'`.**
...
* **قواعد العزل:**
  - `available_balance` يمثل فقط ما هو متاح للشراء الفعلي (الكاش الصافي غير المحجوز + البونص المعتمد).
  - `unclassified_balance` محجوز في المحفظة لحفظ حقوق العميل المحاسبية، لكنه غير قابل للاستهلاك أو السحب حتى يفحصه المشرف ويعيد تصنيفه عبر أمر `wallet:backfill:classify`.

---

## 2. خدمة التحقق الموحدة من الدفع المالي المؤكد وانتقال الحالات (Payment Verification & Reversals)

### 2.1. كود التحقق الموحد الصارم (`PaymentVerificationService::isFullyPaid`):
مبني على أعمدة وتراكيب جداول `invoices` و `orders` المثبتة في حزمة `Webkul\Sales`:
...
class PaymentVerificationService
{
     ...
     * يستند إلى حقل `state` في جدول `invoices` وحالة الطلب `orders.status`.
     */
    public static function isFullyPaid(Invoice $invoice): bool
    {
        ...
        // 3. التحقق الحسابي بدقة bcmath من أن إجمالي الفاتورة الأساسي > 0
        $baseTotalStr = (string) $invoice->base_grand_total;
        if (bccomp($baseTotalStr, '0.0000', 4) <= 0) {
            return false; // الفواتير الصفرية لا تولد كاش باك
        }

        // 4. التحقق من تحصيل الفاتورة فعلياً (base_grand_total_invoiced > 0)
        $invoicedTotalStr = (string) ($order->base_grand_total_invoiced ?? '0.0000');
        if (bccomp($invoicedTotalStr, '0.0000', 4) <= 0) {
            return false;
        }
...
### 2.2. دورة حياة الدفع والعكس (Payment Transitions & Reversals):
1. **انتقال حالة الدفع المؤكد:** عند تحول الفاتورة من `pending` إلى `paid` ⬅️ يُسجل حدث `OrderPaymentConfirmed` في جدول `wallet_promotion_outbox` داخل نفس معاملة الـ DB.
2. **عكس الدفع (Payment Reversal / Chargeback / Refund):**
   - إذا تم رد الفاتورة (`refunded`) أو إنشاء استرداد في جدول `refunds` ⬅️ ينطلق معالج الاسترداد لعكس الكاش باك الممنوح وفق خوارزمية الأصناف المؤهلة.
3. **حماية إعادة الحفظ (Re-save Protection):** المفتاح `order:{order_id}:invoice:{invoice_id}` يمنع تكرار الإطلاق عند أي إعادة حفظ لاحقة للفاتورة.

---

## 3. الخدمة المركزية ومصفوفة حالات شحن المحفظة (Top-Up Service & Transition Matrix)

### 3.1. مصفوفة انتقال الحالات (`WalletTopUp State Transitions`):
| الحالة الحالية | الحالة المستهدفة | العملية | النتيجة |
|---|---|---|---|
| `pending` | `completed` | اعتماد الإيداع من المشرف أو Webhook | **مسموح** ⬅️ قيد الكاش في الـ Ledger وإطلاق حدث البونص |
...
### 3.2. الكود البرمجي المركزي للخدمة (`WalletTopUpService::completeTopUp`):
```php
namespace Webkul\Wallet\Services;
...
class WalletTopUpService
{
    ...
    public function completeTopUp(
        ...
        return DB::transaction(function () use ($topup, $actorType, $actorId, $paymentTxnId) {
            ...
            // التحقق الصارم من مصفوفة الحالات
            if ($lockedTopup->status !== 'pending' && $lockedTopup->status !== 'pending_payment') {
                throw new InvalidWalletTransitionException(
                    "Cannot approve TopUp #{$lockedTopup->id} in status [{$lockedTopup->status}]."
                );
            }
            ...
            // 3. قيد الحدث في Outbox للأمان
            WalletPromotionOutbox::create([
                'event_type' => 'WalletTopUpApproved',
                'event_key'  => "topup:{$lockedTopup->id}:completed",
                'payload'    => [
...
## 4. آلية الـ Outbox الآمنة والحجز الذري (Atomic Outbox Lease & Recovery)

### 4.1. بنية الحقول للحجز والحماية من التزامن:
- `locked_at` (`datetime nullable`)
- `locked_by` (`varchar(100) nullable` - معرّف العامل Worker ID)
- `lease_expires_at` (`datetime nullable` - وقت انتهاء عقد الحجز)
- `attempts` (`int unsigned default 0`)
- `status` (`enum('pending', 'processing', 'completed', 'failed')`)

### 4.2. استعلام الحجز الذري واستعادة المهام العالقة (Atomic Claim Query):
```php
public function claimPendingOutboxJobs(string $workerId, int $limit = 50, int $leaseSeconds = 300): Collection
{
    return DB::transaction(function () use ($workerId, $limit, $leaseSeconds) {
        $now = now()->toDateTimeString();
        $leaseExpiry = now()->addSeconds($leaseSeconds)->toDateTimeString();

        // 1. تحديد المعرفات المتاحة للحجز (الجديدة أو التي انتهى عقد حجزها ولم تكتمل)
        $jobIds = DB::table('wallet_promotion_outbox')
        ...
        // 2. تحديث عقد الحجز ذرياً
        DB::table('wallet_promotion_outbox')
            ->whereIn('id', $jobIds)
            ->update([
                'status'           => 'processing',
                'locked_at'        => $now,
        ...
        return WalletPromotionOutbox::whereIn('id', $jobIds)->get();
    });
}
```
...
## 5. خوارزمية استهلاك الحصص (FIFO Lot Consumption Pseudo-Code)
...
public function consumePromoLots(
    ...
    return DB::transaction(function () use (
        ...
        // جلب الحصص النشطة والصالحة زمنياً فقط مرتبة FIFO مع NULL expiry
        ...
        // تحديث أرصدة المحفظة (مع عزل unclassified_balance التام عن الشراء)
...
## 6. جدول التوافق الفعلي للـ Foreign Keys مع كود و Schema Bagisto 2.4.x
...
## 7. نموذج البيانات النهائي الكامل (Final Complete SQL Schema - V1.6)
...
CREATE TABLE `wallet_promo_debts` (
  ...
  CONSTRAINT `chk_debt_math` CHECK (`remaining_debt_amount` >= 0 AND `settled_amount` >= 0),
  FOREIGN KEY (`wallet_id`) REFERENCES `wallet_accounts` (`id`) ON DELETE RESTRICT,
  FOREIGN KEY (`customer_id`) REFERENCES `customers` (`id`) ON DELETE RESTRICT,
  FOREIGN KEY (`order_id`) REFERENCES `orders` (`id`) ON DELETE RESTRICT,
...
## 8. مصفوفة الاختبارات المعمارية الشاملة والمحدثة (Final Executable Test Matrix)
...
| **T-05** | حالات الدفع المتناقضة | فاتورة تحمل `state = paid` ولكن `base_total_invoiced = 0` | رفض المعاملة فوراً وتجاهل منح الكاش باك | سجل Log يوضح فشل التحقق من الدفع |
...
| **T-12** | عزل الحسابات الملتبسة بالـ Backfill | حساب ملتبس لم تثبت حركاته السابقة | تصنيف الحساب كـ `pending_review` وعزل رصيده ومنع السحب والشراء والبونص | فحص `wallet_accounts.backfill_status` |
...
| **T-15** | رفض اعتماد Top-Up مرفوض سابقاً | محاولة اعتماد طلب شحن تم رفضه مسبقاً (`rejected`) | رمي استثناء ومنع إيداع أي رصيد أو بونص | فحص استثناء `InvalidWalletTransitionException` |

---

## 9. بوابة المراجعة والاعتماد (Review & Approval Gate)

### **حالة العقد النهائي V1.6:** `READY FOR REVIEW & APPROVAL`

**إقرار المهندس المشرف:**
- تم استيفاء وتدقيق كافة التوجيهات الهندسية المحاسبية الـ 10 بدقة متناهية.
- تمت مراجعة كافة المعادلات، الـ Schema الكاملة، آليات الـ Outbox Lease، مصفوفة حالات TopUp، وجدول توافق الـ Foreign Keys بالكامل مع كود Bagisto.
