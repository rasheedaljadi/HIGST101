...
def main():
    ...
    # Generate unique test marker: SIM-PROC-V2-SA-<timestamp>-<suffix>
    ts = datetime.utcnow().strftime('%Y%m%d%H%M%S')
    suffix = secrets.token_hex(3).upper()
    marker = f"SIM-PROC-V2-SA-{ts}-{suffix}"
    
    print(f"[Simulation] Generated Unique Marker: {marker}")
...
// 2. In-memory validation of default warehouse Saudi address
$warehouse = DB::table('inventory_sources')->where('code', 'default')->first();
if (!$warehouse) {
    echo json_encode(['error' => 'GATE_FAILED: default inventory source missing']);
    exit(1);
}

$rawPostcode = trim((string)($warehouse->postcode ?? ''));
$zipLen = strlen($rawPostcode);
$patternValid = (bool)preg_match('/^[A-Z]{4}[0-9]{4}$/', $rawPostcode);

if ($zipLen !== 8 || !$patternValid) {
    echo json_encode([
        'error' => 'GATE_FAILED_SAUDI_ADDRESS_GUARD: default inventory source postcode is invalid',
        'length' => $zipLen,
...
try {
    ...
    $addressGuardPassed = ($validatedAddress instanceof ValidatedAliExpressShippingAddress && $validatedAddress->country === 'SA' && strlen($validatedAddress->zip) === 8);
    $addressSummary = $validatedAddress->getMaskedSummary();
} catch (\\Throwable $e) {
    echo json_encode([
        'error' => 'GATE_FAILED_SAUDI_ADDRESS_GUARD: Exception thrown during validation',
        'message' => $e->getMessage()
    ]);
    exit(1);
}

if (!$addressGuardPassed) {
    echo json_encode(['error' => 'GATE_FAILED_SAUDI_ADDRESS_GUARD: Address validation failed']);
    exit(1);
}

// 3. Verify Product 3163 and Deficit
$variantId = 3163;
...
// 6. Execute Canonical Simulation
...
// Step 6.1: Create isolated test customer
...
// Step 6.2: Create Order for product 3163 via canonical Bagisto structure
    ...
    'items' => [
        [
            ...
            'additional' => [
                ...
                // provider_account_id not specified -> resolves to null
            ],
        ]
    ],
];
...
// Step 6.3: Invoke ProcurementDemandService
...
// Step 6.4: Invoke ProcurementBatchService
$admin = Admin::first();
$adminId = $admin?->id ?? 1;
...
$result = [
    ...
    'address_guard' => [
        ...
        'zip_present' => true,
        'zip_len' => $zipLen,
        'matches_pattern' => $patternValid,
        'zip_masked' => substr($rawPostcode, 0, 2) . '****' . substr($rawPostcode, -2),
        'guard_passed' => $addressGuardPassed,
        'masked_summary' => $addressSummary,
    ],
];

// 7. Baseline Counts After Execution
$countsAfter = [];
foreach ($tables as $t) {
...
$spo36Unchanged = (
    $spo36Before->state === $spo36After->state &&
    $spo36Before->payment_state === $spo36After->payment_state &&
    $spo36After->state === 'supplier_exception'
);

$epo27Unchanged = (
    $epo27Before->raw_status === $epo27After->raw_status &&
    $epo27Before->failure_code === $epo27After->failure_code &&
    $epo27After->external_order_id === null
);
...
echo json_encode([
    ...
    'historical_audit' => [
        ...
        'spo36_unchanged' => $spo36Unchanged,
        'epo27_unchanged' => $epo27Unchanged,
    ]
], JSON_PRETTY_PRINT);
"""
    ...
    remote_script_path = f"{remote_base}/scripts/execute_simulation_proc_v2_sa.php"
    with sftp.open(remote_script_path, 'w') as f:
        f.write(simulation_php)
    sftp.close()
    
    print("[Simulation] Uploaded execute_simulation_proc_v2_sa.php to Staging")
    
    # Execute simulation
    cmd = f"cd {remote_base} && php scripts/execute_simulation_proc_v2_sa.php"
    code, out, err = run_remote_cmd(client, cmd)
    print(f"\n--- Simulation Execution Output ---\n{out}")
    if err:
        print(f"\n--- Simulation Execution STDERR ---\n{err}")
        
    # Remove temp script
    run_remote_cmd(client, f"rm -f {remote_script_path}")
    
    try:
        sim_data = json.loads(out)
        out_json_path = os.path.join(os.path.dirname(os.path.abspath(__file__)), 'third_simulation_execution_result.json')
        with open(out_json_path, 'w', encoding='utf-8') as f:
            json.dump(sim_data, f, indent=2)
        print(f"[Simulation] Saved execution result to {out_json_path}")
    except Exception as e:
        print(f"[ERROR] Could not parse simulation JSON: {e}")
