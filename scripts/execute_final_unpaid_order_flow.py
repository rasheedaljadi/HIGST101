...
$historicalSposBefore = DB::table('supplier_purchase_orders')->whereIn('id', [35, 36, 37])->get()->keyBy('id')->toArray();
$historicalEposBefore = DB::table('external_platform_orders')->whereIn('id', [26, 27, 28])->get()->keyBy('id')->toArray();
...
$approvedBatch = $batchService->approveBatch($batch->id, $adminId, "Approved for final simulation " . $marker);
...
$historicalSposAfter = DB::table('supplier_purchase_orders')->whereIn('id', [35, 36, 37])->get()->keyBy('id')->toArray();
$historicalEposAfter = DB::table('external_platform_orders')->whereIn('id', [26, 27, 28])->get()->keyBy('id')->toArray();
...
"""
    ...
    remote_script_path = f"{remote_base}/scripts/execute_final_unpaid_order_flow.php"
    with sftp.open(remote_script_path, 'w') as f:
        f.write(execution_php)
    sftp.close()
    
    print("[Single-Command Execution] Uploaded execute_final_unpaid_order_flow.php to Staging")
    print("[Single-Command Execution] Starting complete pipeline...")
    
    cmd = f"cd {remote_base} && php scripts/execute_final_unpaid_order_flow.php"
    ...
    try:
        ...
        out_json_path = os.path.join(os.path.dirname(os.path.abspath(__file__)), 'final_unpaid_order_execution_result.json')
