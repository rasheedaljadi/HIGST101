...
def main():
    ...
    marker = f"SIM-PROC-V2-{ts}-{suffix}"
...
// Step 5.1: Create isolated non-contactable test customer
$defaultGroup = $customerGroupRepo->findOneByField('code', 'general') ?? $customerGroupRepo->first();
$customerEmail = 'simulation_' . strtolower(str_replace('-', '_', $marker)) . '@highest-internal.test';
...
echo json_encode([
    'ruling' => $invariantsPassed ? 'LEGITIMATE_SIMULATION_SPO_READY_FOR_FRESH_PREFLIGHT' : 'SIMULATION_BLOCKED_OR_FAILED — Invariants check failed',
    'marker' => $marker,
    'git_sha' => $gitSha,
    'git_clean' => true,
    'app_debug' => $appDebug,
...
"""
    ...
    with sftp.file('/tmp/execute_simulation.php', 'w') as f:
        f.write(simulation_php)
    sftp.close()
    
    code, out, err = run_remote_cmd(client, "php /tmp/execute_simulation.php && rm -f /tmp/execute_simulation.php")
    print(out)
    if err:
        print("ERR:", err)
