...
print("[Deploy] Uploading updated files...")
...
print("\n=======================================================")
print("VERIFYING INSTANT SHIPPING SELECTION SYNC")
print("=======================================================")
...
echo "\n--- Step 1: Customer selects Home Delivery ($5.00) ---\n";
...
echo "\n--- Step 2: Customer switches to Delivery Point ($0.00) ---\n";
...
$respPickup = $controller->storeShippingMethod();
$pickupData = json_decode($respPickup->content(), true);

echo "Cart shipping_amount: " . ($pickupData['cart']['shipping_amount'] ?? 'NULL') . "\n";
echo "Cart formatted_shipping_amount: " . ($pickupData['cart']['formatted_shipping_amount'] ?? 'NULL') . "\n";
echo "Cart grand_total: " . ($pickupData['cart']['grand_total'] ?? 'NULL') . "\n";
echo "Cart formatted_grand_total: " . ($pickupData['cart']['formatted_grand_total'] ?? 'NULL') . "\n";
"""

sftp = client.open_sftp()
with sftp.file(f"{REMOTE_ROOT}/test_verify_instant_shipping.php", "w") as f:
    f.write(php_test)
sftp.close()

code, out, err = r.run_remote_cmd(client, f"cd {REMOTE_ROOT} && php8.4 test_verify_instant_shipping.php && rm test_verify_instant_shipping.php")
print(f"\nTEST RESULTS:\n{out}")
if err:
    print(f"ERR:\n{err}")
