...
use Webkul\\Procurement\\Models\\ProcurementSupplierOrder;

echo "=========================================================\\n";
echo "1. FINDING MP3 PRODUCT & ITS VARIANTS\\n";
echo "=========================================================\\n";

$products = Product::where('name', 'like', '%MP3%')
    ->orWhereHas('attribute_values', function($q) {
        $q->where('text_value', 'like', '%MP3%');
    })->get();

foreach ($products as $p) {
    echo "Product #{$p->id} (Type: {$p->type}, SKU: {$p->sku}): {$p->name}\\n";
    ...
    if ($p->type === 'configurable') {
        foreach ($p->variants as $v) {
            ...
            echo "    Variant #{$v->id} (SKU: {$v->sku}, Local Qty: {$inv}):\\n";
            if ($proj) {
...
with sftp.file("/home/highest-ye/htdocs/highest-ye.store/inspect_mp3_product.php", "w") as f:
    f.write(php_test)
sftp.close()

code, out, err = r.run_remote_cmd(client, "cd /home/highest-ye/htdocs/highest-ye.store && php8.4 inspect_mp3_product.php && rm inspect_mp3_product.php")
print(f"OUT:\n{out}")
