...
$controller = new class(
    ...
    public function findCulprit() {
        ...
        foreach ($records->take(10) as $idx => $c) {
            ...
            <table><thead><tr><th>ID</th><th>الاسم</th><th>البريد</th></tr></thead>
            <tbody>
            <tr><td>' . $c->customer_id . '</td><td>' . htmlspecialchars($c->name) . '</td><td>' . htmlspecialchars($c->email) . '</td></tr>
            </tbody></table></body></html>';
            
            $mpdf = new \\Mpdf\\Mpdf(['mode' => 'utf-8', 'format' => 'A4-L', 'orientation' => 'L']);
            $mpdf->WriteHTML($html);
            echo "Customer #{$c->customer_id} single row => {$mpdf->page} pages\\n";
        }
    }
};

$controller->findCulprit();
"""

sftp = client.open_sftp()
with sftp.file(f"{APP_DIR}/test_find_culprit.php", 'w') as f:
    f.write(test_script)
sftp.close()

stdin, stdout, stderr = client.exec_command(f"cd {APP_DIR} && php test_find_culprit.php")
print(stdout.read().decode('utf-8', errors='replace'))
print(stderr.read().decode('utf-8', errors='replace'))

client.exec_command(f"rm {APP_DIR}/test_find_culprit.php")
client.close()
