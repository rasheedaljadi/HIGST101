...
// 1. Verify Git Invariants
$gitSha = trim(shell_exec('cd ' . escapeshellarg($projDir) . ' && git rev-parse HEAD'));
$gitStatus = trim(shell_exec('cd ' . escapeshellarg($projDir) . ' && git status --short'));
...
$gitClean = ($gitSha === '{target_sha}' && $fileSha === $blobSha);
...
echo json_encode([
    'ruling' => $isReady ? 'GIT_CLEAN_STRICT_LIVE_PREAPPROVAL_READY' : 'STRICT_PREAPPROVAL_BLOCKED',
...
"""
    ...
    with sftp.file('/tmp/execute_git_clean_live_preflight.php', 'w') as f:
        f.write(preflight_php)
    sftp.close()
    
    code, out, err = run_remote_cmd(client, "php /tmp/execute_git_clean_live_preflight.php && rm -f /tmp/execute_git_clean_live_preflight.php")
    print(out)
    if err:
        print("ERR:", err)
