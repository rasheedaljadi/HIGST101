...
$products = \App\Models\AliExpressProductImport::whereNotNull('product_id')->get();
echo "Found " . $products->count() . " imported AliExpress products.\n\n";
...
foreach ($products as $p) {
    ...
    echo "  - Current Catalog Price: " . ($bagistoProduct ? $bagistoProduct->price : 'N/A') . "\n";
    if ($history && $history->breakdown_json) {
        $freight = $history->breakdown_json['freight_adjustment'] ?? null;
        echo "  - Last Calculation Freight Breakdown: " . json_encode($freight, JSON_UNESCAPED_UNICODE) . "\n";
    }
    echo "-------------------------------------------\n";
}
